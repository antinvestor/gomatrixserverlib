...
func TestVerifyEventAuthChain(t *testing.T) {
	...
	testEvent, _ := gomatrixserverlib.MustGetRoomVersion(gomatrixserverlib.RoomVersionV1).
		NewEventFromTrustedJSON(testEvents[len(testEvents)-1], false)
...
func TestVerifyEventAuthChainLying(t *testing.T) {
	...
	testEvents := [][]byte{
		...
		[]byte(
			`{"auth_events":[["$WCraVpPZe5TtHAqs:baba.is.you",{"sha256":"gBxQI2xzDLMoyIjkrpCJFBXC5NnrSemepc7SninSARI"}]],"content":{"body":"Test Message"},"depth":3,"event_id":"$4Kp0G1yWZ6tNpeI7:baba.is.you","hashes":{"sha256":"B+MjcGZRh72iaGOgyNbIxgFkHDJo6NO8NQDgiKDKDBA"},"origin":"baba.is.you","origin_server_ts":0,"prev_events":[["$xOJZshi3NeKKJiCf:baba.is.you",{"sha256":"5PGENImHC863Yz9sO6IJX+bIQthZFI2RMhFZyFy+bC0"}]],"room_id":"!roomid:baba.is.you","sender":"@userid:baba.is.you","signatures":{"baba.is.you":{"ed25519:auto":"rP+Ybp17GPCqQBrTQ3yz+q6PihdaMWvNY3SngV8aDLHv8wdDlH4ULGnjsB+Az7trqYdCE3rZVo9M7Hy5tOObDg"}},"type":"m.room.message"}`,
		),
	}
	testEvent, _ := gomatrixserverlib.MustGetRoomVersion(gomatrixserverlib.RoomVersionV1).
		NewEventFromTrustedJSON(testEvents[len(testEvents)-1], false)
...
func provideEvents(t *testing.T, events [][]byte) gomatrixserverlib.EventProvider {
	...
	for _, eventBytes := range events {
		ev, err := gomatrixserverlib.MustGetRoomVersion(gomatrixserverlib.RoomVersionV1).
			NewEventFromTrustedJSON(eventBytes, false)
		if err != nil {
			t.Fatalf("Failed to load event: %s", err)
		}
		eventMap[ev.EventID()] = ev
	}
	return func(ctx context.Context, roomVer gomatrixserverlib.RoomVersion, eventIDs []string) (result []gomatrixserverlib.PDU, err error) {
